...
// Verifica daca un user este admin, returneaza true/false
...
module.exports = {
    // Returneaza toti userii (doar pentru admin)
    ...
    // Sterge user dupa id (nu poti sterge userul tau propriu)
    async usersDelete(req, res, user) {
        ...
        req.on("end", async () => {
            ...
            if (!id || id == user.id) 
                return sendResponse(res, 400, "application/json", JSON.stringify({error:"Invalid user id."}));
            await pool.query("DELETE FROM users WHERE id = $1", [id]);
            sendResponse(res, 200, "application/json", JSON.stringify({success:true}));
        });
    },

    // Returneaza toate review-urile (doar pentru admin)
    ...
    // Sterge review dupa id (doar pentru admin)
    async reviewsDelete(req, res, user) {
        ...
        req.on("end", async () => {
            const { id } = JSON.parse(body);
            if (!id) 
                return sendResponse(res, 400, "application/json", JSON.stringify({error:"Invalid review id."}));
    ...
    // Returneaza toate bug report-urile (doar pentru admin)
    async bugReportsGet(req, res, user) {
        if (!user || !(await isAdminUser(user.id))) 
            return sendResponse(res, 403, "application/json", JSON.stringify({error:"Forbidden"}));
        const bugs = await pool.query(
            "SELECT bug_reports.*, users.username FROM bug_reports LEFT JOIN users ON bug_reports.user_id = users.id ORDER BY created_at DESC"
        );
        return sendResponse(res, 200, "application/json", JSON.stringify(bugs.rows));
    },

    // Sterge un bug report dupa id (doar pentru admin)
    async bugReportsDelete(req, res, user) {
        ...
        req.on("end", async () => {
            const { id } = JSON.parse(body);
            if (!id) 
                return sendResponse(res, 400, "application/json", JSON.stringify({error:"Invalid bug report id."}));
